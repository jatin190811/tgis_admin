...
function Home() {
  ...
  useEffect(() => {
    axios.post(url + 'mehndi', {}).then(resp => {
      setState({ ...state, mehndis: resp?.data?.data })
    })
  }, [])
  ...
  const deleteItemConfirm = (i) => {
    ...
    axios.post(url + 'mehndi/delete/' + state.deleteRef._id, {}, {
      headers: {
      ...
      if (resp.data.status != 'error') {
        axios.post(url + 'mehndi', {}).then(resp => {
          setState({ ...state, mehndis: resp?.data?.data,  deleteRef: null, showDeleteModal: false, })
            ...
            <div className='working-area'>
              ...
              <div className='row'>
                <div className='col-12'>
                  <button onClick={()=>navigate('/mehndi/add')} className='btn btn-primary pull-right'>Add New</button><br /><br />
                </div>
                {state.mehndis && state.mehndis.length ? <div class="table-responsive">
                            ...
                            <tr>
                              ...
                              <td>
                                <button className='btn btn-primary btn-sm' onClick={() => {
                                  navigate('/mehndi/' + item._id)
                                }}>View</button>&nbsp;
                                <button className='btn btn-warning btn-sm' onClick={() => {
                                  navigate('/mehndi/media/' + item._id)
                                }}>Media</button>&nbsp;
                                 <button className='btn btn-warning btn-sm' onClick={() => {
                                  navigate('/mehndi/update/' + item._id)
                                }}>Update</button>&nbsp;
