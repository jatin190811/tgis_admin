...
function Home() {
  ...
  useEffect(() => {
    axios.post(url + 'photographers', {}).then(resp => {
      setState({ ...state, photographers: resp?.data?.data })
    })
  }, [])
  ...
  const deleteItemConfirm = (i) => {
    ...
    axios.post(url + 'photographer/delete/' + state.deleteRef._id, {}, {
      headers: {
      ...
      if (resp.data.status != 'error') {
        axios.post(url + 'photographers', {}).then(resp => {
          setState({ ...state, photographers: resp?.data?.data,  deleteRef: null, showDeleteModal: false, })
            ...
            <div className='working-area'>
              ...
              <div className='row'>
                <div className='col-12'>
                  <button onClick={()=>navigate('/photographer/add')} className='btn btn-primary pull-right'>Add New</button><br /><br />
                </div>
                {state.photographers && state.photographers.length ? <div class="table-responsive">
                  ...
                  <table class="table table-hover table-bordered table table-striped">
                    ...
                    <tbody>
                      {
                        state.photographers.map((item, index) => {
                          return (
                            <tr>
                              <td>{item.name}</td>
                              <td>{item.type}</td>
                              ...
                              <td>
                                <button className='btn btn-primary btn-sm' onClick={() => {
                                  navigate('/photographer/' + item._id)
                                }}>View</button>&nbsp;
                                <button className='btn btn-warning btn-sm' onClick={() => {
                                  navigate('/photographer/media/' + item._id)
                                }}>Media</button>&nbsp;
                                 <button className='btn btn-warning btn-sm' onClick={() => {
                                  navigate('/photographer/update/' + item._id)
                                }}>Update</button>&nbsp;
